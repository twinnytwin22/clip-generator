...
class DownloadRequest(BaseModel):
    vod_url: str
    title: str
    storage: str = "local"        # "local" or "supabase"
    quality: str = "720p" 
    user_id: str        # e.g. "720p", "480p", etc.
...
async def download_vod(req: DownloadRequest):
    ...
    # 3. Pick requested quality (fallback to best)
    ...
    # 4. Download TS into output folder
    ...
    if req.storage.lower() == "supabase":
        ...
        with open(mp4_path, "rb") as f:
            data = f.read()
        res = supabase.storage.from_(SUPABASE_BUCKET).upload(f"{req.user_id}/{vod_id}.mp4", data)
        if res.get("error"):
            logger.error("✗ Supabase upload error: %s", res['error']['message'])
            raise HTTPException(500, f"Supabase upload failed: {res['error']['message']}")
        url = supabase.storage.from_(SUPABASE_BUCKET).get_public_url(f"{req.user_id}{vod_id}.mp4")["publicURL"]
        logger.info("✔ Uploaded, public URL: %s", url)
        return {"url": url}

    # 6b. Return local path
    logger.info("✔ Finished: returning local path %s", mp4_path)
